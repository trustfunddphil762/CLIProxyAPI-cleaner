...
5. Docker 模式下控制台现在会显式使用 `/app/docker/supervisord.conf` 调 `supervisorctl`，并且 `supervisord.conf` 已启用 `[rpcinterface:supervisor]`，避免误连系统默认 socket 或出现 supervisor namespace 不可用的报错。
