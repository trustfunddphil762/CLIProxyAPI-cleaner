...
async function viewReport(name) {
  try {
    const data = await api(`/proxy-cleaner/api/report?name=${encodeURIComponent(name)}`);
...
async function refreshStatus(silent = false) {
  try {
    const data = await api('/proxy-cleaner/api/status');
...
$('#loginForm').addEventListener('submit', async (e) => {
  ...
  try {
    await api('/proxy-cleaner/api/login', { method: 'POST', body, form: true });
...
$('#logoutBtn').addEventListener('click', async () => {
  try { await api('/proxy-cleaner/api/logout', { method: 'POST', body: {} }); } catch (_) {}
  state.loggedIn = false;
  $('#loginCard').classList.remove('hidden');
  $('#app').classList.add('hidden');
  $('#logoutBtn').classList.add('hidden');
...
$('#configForm').addEventListener('submit', async (e) => {
  ...
  try {
    const data = await api('/proxy-cleaner/api/config/save', { method: 'POST', body: payload });
...
document.querySelectorAll('[data-action]').forEach((btn) => {
  btn.addEventListener('click', async () => {
    ...
    try {
      const data = await api(`/proxy-cleaner/api/service/${action}`, { method: 'POST', body: { target } });
      renderStatus(data.data);
...
$('#runOnceBtn').addEventListener('click', async () => {
  try {
    const data = await api('/proxy-cleaner/api/run-once', { method: 'POST', body: {} });
